...
This is still a work in progress. **I expect to have a link to the github repo by mid February** once I am satisfied with the work. At the moment, I have designed a working model that can give decent predictions with roughly 60% accuracy on the validation set (roughly 60% specificity and 60% sensitivity). I hope to improve this to > 70% as I feed my models more data (I have only used 15% of the available data thus far, but it will take time to gather and clean), try out different ML techniques and do hyperparameter tuning. Training was done using Google Colab's free GPU's. As a comparison, my own human predictive accuracy is roughly 70%-80%, and someone who does not play pool or is a beginner would almost certainly perform worse than the model. 
